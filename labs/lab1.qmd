...
5.  \*\*Finally, add the \_quarto.yml file\*\*:
    -   Save this `quarto.yml` file in soc `labs` folder
    -   Make sure the `_quarto.yml` file is in the `soc16` and not `data` or `lab` folders. Your folder structure should look like this:
...
First, install these packages in your console (only need to do this once):

```{r}
#| eval: false

# Run these in the console ONCE to install
...
The `slice()` function selects specific rows by their position (row number). The colon `:` here means through.
...
The `filter()` function selects rows based on conditions. You'll likely want to use some logical operator here (e.g., `>` or `==`)
