...
n_sim <- 300
sim_skin <- tibble(
  skin_tone   = runif(n_sim, 1, 10),
  income_z    = rnorm(n_sim),
  self_esteem = 4.5 + 0.30 * skin_tone + 0.08 * income_z -
                0.04 * skin_tone * income_z + rnorm(n_sim, 0, 1.2)
)
model_skin <- lm(self_esteem ~ skin_tone * income_z, data = sim_skin)
...
Both X₁ and X₂ are continuous (e.g., age × education → income).
...
::: {.callout-note icon=false}
## Evaluating at meaningful values
In a continuous × continuous interaction, $\beta_1$ is the effect of $X_1$ **only when $X_2 = 0$** — which is often outside the range of your data and substantively meaningless. Instead, compute **simple slopes**: the effect of $X_1$ at low, average, and high values of $X_2$ (conventionally mean − 1 SD, mean, mean + 1 SD). Use `ggpredict(model, terms = c("X1", "X2 [meansd]"))` to compute and plot these automatically.
:::

::: {.callout-note icon=false}
## Evaluating direction:
More common but less precise is to just describe the direction. For example, "As $X_2$ increases, the impact of $X_1$ increases/decreases (depending on +/- of $X_3$)".
...
**Model:** `lm(self_esteem ~ skin_tone * income_z)` — does income moderate the impact of skin-tone on self-esteem?
...
modelsummary(
  ...
  coef_rename = c("(Intercept)"        = "Intercept",
                  "skin_tone"          = "Skin Tone",
                  "income_z"           = "Income (std.)",
                  "skin_tone:income_z" = "Skin Tone × Income"),
  stars   = TRUE,
  fmt     = 2,
  title   = "OLS: Self-Esteem ~ Skin Tone × Income",
  gof_map = c("nobs", "r.squared")
)
```

One effective way to interpret the interaction is to calculate the effect size at **low, average, and high values** of income (e.g., low = −1 SD, mean = 0, high = +1 SD) (assuming income is standardized):

| Income level | Calculation | Skin-tone slope |
|---|---|---|
| Low (−1 SD) | $0.30 + (-0.04)(-1)$ | **+0.34** |
| Mean (0) | $0.30 + (-0.04)(0)$ | **+0.30** |
| High (+1 SD) | $0.30 + (-0.04)(+1)$ | **+0.26** |

::: {.callout-tip icon=false}
## Interpretation at meaningful values
The simple slopes show that the positive effect of skin tone on self-esteem is strongest at **low income** (+0.34) and attenuates as income rises (+0.26 at high income). In other words, income **moderates** the impact of skin-tone on self-esteem: higher income weakens the relationship. $\beta_3 = -0.04$ tells us the skin-tone slope decreases by 0.04 for each 1-SD increase in income.
:::

::: {.callout-tip icon=false}
## Interpretation using direction
- "As income increases, the effect of skin tone descreases."
- Or something a bit more precise: "At lower values of income, skin tone has a stronger positive association with self-esteem (+0.34), but as income increases the impact of skin tone on self-esteem weakens (+0.26)."
:::
...
::: {.callout-tip icon=false}
## Interpretation
Read `Education × Female` from the table: that is $\beta_3$, the difference in returns per year of education between women and men. If negative, women gain less per additional year. Note: `Female` ($\beta_2$) is the income gap at zero years of education — rarely meaningful on its own.
:::
...
| | White ($X_1=0$) | Black ($X_1=1$) |
|---|---|---|
| **Men** ($X_2=0$) | $\$65K$ | $\$65K-\$12K = \$53K$ |
| **Women** ($X_2=1$) | $\$65K -\$12K  = \$47K$ |$\$65K -\$12K -\$18K +\$5K= \$40K$ |

**Race gap within sex:**

- Among men: \$65,000 − \$53,000 = **−\$12,000** ($\beta_1$)
- Among women: \$47,000 − \$40,000 = **−\$7,000** ($\beta_1 + \beta_3$)

**Gender gap within race:**

- Among white workers: \$65,000 − \$47,000 = **−\$18,000** ($\beta_2$)
- Among Black workers: \$53,000 − \$40,000 = **−\$13,000** ($\beta_2 + \beta_3$)

**Race + gender gap (white men vs. Black women):**

- \$65,000 − \$40,000 = **−\$25,000** ($\beta_1 + \beta_2 + \beta_3 = -12{,}000 + (-18{,}000) + 5{,}000$)

::: {.callout-tip icon=false}
## Interpretation
The racial income gap is smaller for women (−\$7k) than for men (−\$12k), and the gender gap is smaller among Black workers (−\$13k) than among white workers (−\$18k). Both patterns are driven by the same thing: $\beta_3 = +5{,}000$. Black women face the largest combined disadvantage at −\$25,000 relative to white men.
:::
...
**Model:** `lm(self_esteem ~ skin_tone * income_z)` — does income moderate the skin-tone → self-esteem relationship?
...
ggpredict(model_skin, terms = c("skin_tone", "income_z [meansd]")) |>
  ggplot(aes(x = x, y = predicted, color = group, fill = group)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.15, color = NA) +
  geom_line(linewidth = 1.4) +
  scale_color_manual(
    values = c("#E15759", "#4E79A7", "#59A14F"),
    labels = c("Low income (−1 SD)", "Mean income", "High income (+1 SD)"),
    name   = "Income level"
  ) +
  scale_fill_manual(
    values = c("#E15759", "#4E79A7", "#59A14F"),
  ...
  labs(
    title    = "Predicted Self-Esteem by Skin Tone and Income",
    subtitle = "Slopes attenuate as income rises — income moderates the skin-tone effect",
    x = "Skin Tone (1 = darkest, 10 = lightest)",
    y = "Predicted Self-Esteem"
  ) +
...
::: {.callout-tip icon=false}
## Reading the plot
The three lines represent the simple slope of skin tone at low (−1 SD), mean, and high (+1 SD) income. The lines diverge: at low income the slope is steepest (+0.34), flattening as income rises (+0.26 at high income). Income **attenuates** the skin-tone effect — the moderator weakens the relationship.
:::
...
::: {.callout-tip icon=false}
## Reading the plot
The two lines show the income slope for men (blue) and women (red) across years of education. A steeper men's line means each additional year of school pays off more for men. Overlapping confidence ribbons signal uncertainty — the interaction term's p-value is the formal test of whether the slopes truly differ.
:::
...
::: {.callout-tip icon=false}
## Reading the plot
The **slope of the line** (drop from White to Black) represents the racial income gap. If the lines are parallel, the gap is identical for men and women — no interaction. Non-parallel lines mean the race penalty (or bonus) differs by sex. Here the gap is smaller for women than for men ($\beta_3 = +5{,}000$), meaning Black women face *less additional* racial penalty compared to Black men — but both lines are low relative to white men (intersectional disadvantage).
:::
