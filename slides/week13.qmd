...
$$E[Y \mid X_1, X_2] = \alpha + \beta_1 X_1 + \beta_2 X_2 + \beta_3 (X_1 \times X_2)$$
...
$$E[Y] = \alpha + \beta_1 X_1 + \beta_2 X_2 + \beta_3 (X_1 \times X_2)$$
...
Because main effects ($\beta_1$, $\beta_2$) represent effects at X = 0, which is often not meaningful, always evaluate the interaction at **substantively interesting values** of the moderating variable (e.g., mean ± 1 SD, or specific theoretical thresholds).

Use `ggpredict(model, terms = c("X1 [meansd]", "X2 [meansd]"))` to plot predicted values at mean ± 1 SD automatically.
:::
...
- Thompson & Keith (2001) use this logic: skin tone × weight predicting men's self-esteem (both continuous)
