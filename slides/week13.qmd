...
- <span class="highlight">April 28</span> — 7–10 minutes per student will give you slide template 
...
Modern methods designed for this problem include: **difference-in-differences** (exploiting before/after comparisons), **regression discontinuity** (exploiting thresholds), and **instrumental variables** (finding exogenous variation in X). This course builds the logic; methods courses go deeper. For now: controlling for observed confounders is good practice, but *"I controlled for everything"* is always a theoretical claim, not a statistical guarantee.
:::
:::
...
The added variable (partial regression) plot shows the education–income relationship *after* removing the variation explained by sex:
...
# Residualize both income and education on sex
resid_inc  <- resid(lm(income91 ~ sex_f, data = attain_sub))
resid_educ <- resid(lm(educ    ~ sex_f, data = attain_sub))

tibble(educ_net_sex = resid_educ, income_net_sex = resid_inc) |>
  ggplot(aes(x = educ_net_sex, y = income_net_sex)) +
  geom_point(alpha = 0.25, color = "gray50", size = 1.2) +
  geom_smooth(method = "lm", color = "#4E79A7", linewidth = 1.4, se = TRUE) +
  labs(
    title   = "Partial regression plot: education → income, net of sex",
    subtitle = "Each axis is residualized — sex's influence removed from both variables",
    x = "Education (residualized on sex)",
    y = "Income (residualized on sex)"
  ) +
...
$$\hat{Y} = 4.5 + 0.30\,(\text{skin_tone}) + 0.08\,(\text{income}) - 0.04\,(\text{skin_tone} \times \text{income})$$

On effective way to visualize this effect is to calculate effect size at **low, average, and high values** of income (e.g., low = −1 SD, mean = 0, high = +1 SD) (assuming income is standardized):
...
**Model:** `lm(income ~ educ * female)` — does the income return to education differ by sex?

$$\hat{Y} = 10{,}000 + 3{,}200\,(\text{educ}) - 8{,}000\,(\text{female}) - 600\,(\text{educ} \times \text{female})$$

| Group | Slope for educ | Calculation |
|---|---|---|
| Men ($\text{female}=0$) | **$3,200/year** | $\beta_1 = 3{,}200$ |
| Women ($\text{female}=1$) | **$2,600/year** | $\beta_1 + \beta_3 = 3{,}200 + (-600)$ |
| Difference | **−$600/year** | $\beta_3 = -600$ |

::: {.callout-tip icon=false}
## Interpretation
Each additional year of education is associated with **$3,200 more income for men** but only **$2,600 more for women** — a gap of $600 per year. If $\beta_3$ is statistically significant (p < .05), we conclude the returns to education differ by sex. Note: $\beta_2 = -8{,}000$ is the gap at **zero years of education** (rarely meaningful on its own).
:::
...
$$\hat{Y} = 65{,}000 - 12{,}000\,(\text{Black}) - 18{,}000\,(\text{female}) + 5{,}000\,(\text{Black} \times \text{female})$$
...
## Visualizing the Interaction {.smaller .scrollable}
...
pred_no_int <- ggpredict(model_main,     terms = c("educ [4:20 by=0.5]", "sex_f"))
pred_int    <- ggpredict(model_interact, terms = c("educ [4:20 by=0.5]", "sex_f"))

make_pred_plot <- function(pred_df, title_str, subtitle_str) {
...
p1 <- make_pred_plot(pred_no_int,
                     "No interaction: parallel slopes",
                     "lm(income91 ~ educ + sex_f)")
p2 <- make_pred_plot(pred_int,
                     "With interaction: diverging slopes",
                     "lm(income91 ~ educ * sex_f)")
...
**Line gap at each X value:** The vertical distance between men's and women's predicted incomes. If the lines are parallel (same slope), the gap is constant. If men's line is steeper, the gap **widens** with more education.

**Confidence ribbons:** Overlapping ribbons suggest uncertainty about whether the two slopes are truly different. The formal test is the interaction term's p-value — not visual ribbon comparison.

**For your data:** Run the same visualization replacing `sex_f` with your moderating variable. Does the X → Y relationship look different across levels of Z?
:::
...
- Apply interaction analysis to your own research dataset
- Practice computing and interpreting group-specific slopes
