...
b_prestg80_main  <- round(coef(model_prestige)[["prestg80"]], 2)
b_age_main       <- round(coef(model_prestige)[["age"]], 2)
mean_age         <- round(mean(attain_sub$age, na.rm = TRUE), 1)
mean_prestg80    <- round(mean(attain_sub$prestg80, na.rm = TRUE), 1)
effect_prest_at_mean_age  <- round(b_prestg80_main + b_prest_age * mean_age, 2)
effect_age_at_mean_prest  <- round(b_age_main + b_prest_age * mean_prestg80, 2)
```
...
$$\text{Effect of } X_1 \text{ for key group: } \hat\beta_1 + \hat\beta_3$$
...
| $X_2 = 1$ (key group) | $\beta_1 + \beta_3$ | Reference slope + interaction |
...
**Main effect of Occupational Prestige** ($\beta_1$ = `r b_prestg80_main`): this is the effect of prestige on income *when age = 0* — a meaningless baseline. At the sample mean age (`r mean_age` years), a one-unit increase in prestige is associated with **\$`r effect_prest_at_mean_age` more income** (`r b_prestg80_main` + `r b_prest_age` × `r mean_age`).

**Main effect of Age** ($\beta_2$ = `r b_age_main`): this is the effect of age on income *when prestige = 0* — again, not meaningful on its own. At the sample mean prestige score (`r mean_prestg80`), each additional year of age is associated with **\$`r effect_age_at_mean_prest` `r ifelse(effect_age_at_mean_prest >= 0, "more", "less")` income** (`r b_age_main` + `r b_prest_age` × `r mean_prestg80`).

**Interaction** ($\beta_3$ = `r b_prest_age`): the prestige → income slope **`r b_prest_age_dir`** as workers get older — prestige **`r b_prest_age_word`** over a career. In the margins plot on the next slide, this will appear as "steeper lines for older ages".

**Keep in mind, however, this interaction is not statistically significant.**
:::
...
**Main effect of Education** ($\beta_1$ = `r scales::dollar(b_int_educ)`): the income return to one additional year of education **for men** (the reference group). Because $X_2 = 0$ for men, this is a straightforward slope — men gain `r scales::dollar(b_int_educ)` per year of schooling.

**Main effect of Female** ($\beta_2$ = `r scales::dollar(b_int_sex)`): the income gap between women and men **when education = 0** — the vertical distance between the two regression lines at their origin. This baseline intercept shift is not a substantively meaningful quantity on its own; use predicted values at realistic education levels to describe the actual income gap.

**Interaction** ($\beta_3$ = `r scales::dollar(b_int_term)`): women's income return per additional year of education is **`r scales::dollar(abs(b_int_term))` `r ifelse(b_int_term < 0, "less", "more")`** than men's. Men gain `r scales::dollar(effect_men)` per year; women gain `r scales::dollar(effect_women)` — a difference that is `r sig_verdict` (`r p_int_label`).
...
**Main effect of Race** ($\beta_1$ = −\$`r f_black`): the income gap between white and Black men (the reference group for sex is men). Among men, Black workers earn \$`r f_race_men` less than white workers.

**Main effect of Sex** ($\beta_2$ = −\$`r f_women`): the income gap between white men and white women (the reference group for race is white). Among white workers, women earn \$`r f_sex_white` less than men.

**Interaction** ($\beta_3$ = \$`r f_int`, `r p_bb_label`): the additional income difference for Black women beyond what the two main effects predict additively. A positive $\beta_3$ means the combined penalty is *smaller* than the sum of the two gaps; negative means it is *larger*. This term is **`r sig_bb`**, so we cannot conclude that the racial income gap is meaningfully different for women versus men.

**High-level summary:** Both race and sex are independently associated with lower income. Black workers earn less than white workers, and women earn less than men. The interaction term suggests the two penalties may not simply add together for Black women, but this non-additivity does not clear the significance threshold — treat the main effects as the primary finding.
:::
...
p_logit_int     <- summary(model_logit_int)$coefficients["educ:sex_ffemale", "Pr(>|z|)"]
p_logit_label   <- ifelse(p_logit_int < 0.001, "p < 0.001",
                   ifelse(p_logit_int < 0.01,  "p < 0.01",
                   ifelse(p_logit_int < 0.05,  "p < 0.05", paste0("p = ", round(p_logit_int, 3)))))
sig_logit       <- ifelse(p_logit_int < 0.05, "statistically significant", "not statistically significant")
b_logit_int     <- coef(model_logit_int)[["educ:sex_ffemale"]]
or_logit_int    <- round(exp(b_logit_int), 3)
logit_int_dir   <- ifelse(b_logit_int < 0, "weaker for women than for men", "stronger for women than for men")

# Main effect ORs and p-values
or_logit_educ   <- round(exp(coef(model_logit_int)[["educ"]]), 3)
or_logit_female <- round(exp(coef(model_logit_int)[["sex_ffemale"]]), 3)

p_logit_educ    <- summary(model_logit_int)$coefficients["educ", "Pr(>|z|)"]
p_logit_educ_label <- ifelse(p_logit_educ < 0.001, "p < 0.001",
                      ifelse(p_logit_educ < 0.01,  "p < 0.01",
                      ifelse(p_logit_educ < 0.05,  "p < 0.05", paste0("p = ", round(p_logit_educ, 3)))))
sig_logit_educ  <- ifelse(p_logit_educ < 0.05, "statistically significant", "not statistically significant")

p_logit_female  <- summary(model_logit_int)$coefficients["sex_ffemale", "Pr(>|z|)"]
p_logit_female_label <- ifelse(p_logit_female < 0.001, "p < 0.001",
                        ifelse(p_logit_female < 0.01,  "p < 0.01",
                        ifelse(p_logit_female < 0.05,  "p < 0.05", paste0("p = ", round(p_logit_female, 3)))))
sig_logit_female <- ifelse(p_logit_female < 0.05, "statistically significant", "not statistically significant")
```

::: {.callout-note icon=false}
## Interpretation

**Main effect of Education** (OR = `r or_logit_educ`, `r p_logit_educ_label`): for men (the reference group), each additional year of education multiplies the odds of union membership by `r or_logit_educ`. An OR `r ifelse(or_logit_educ > 1, "> 1 means education is positively associated with union membership among men", "< 1 means education is negatively associated with union membership among men")`. This effect is **`r sig_logit_educ`**.

**Main effect of Female** (OR = `r or_logit_female`, `r p_logit_female_label`): the odds ratio comparing women to men **when education = 0** — a baseline intercept shift with no direct substantive meaning. Interpret the sex difference through predicted probabilities at realistic education values instead. This effect is **`r sig_logit_female`**.

**Interaction: Education × Female** (OR = `r or_logit_int`, `r p_logit_label`): the ratio of education's OR for women to men. An OR `r ifelse(or_logit_int < 1, "< 1 means each year of education multiplies women's odds by *less* than men's — the education effect is weaker for women", "> 1 means each year of education multiplies women's odds by *more* than men's — the education effect is stronger for women")`. This interaction is **`r sig_logit`**.

**High-level summary:** Education `r ifelse(or_logit_educ > 1, "increases", "decreases")` the odds of union membership for men. The interaction term tells us whether that effect differs for women — since it's positive, the effect of education is positive for women, which we'll see clearly in the margins plot.
:::
...
The interaction OR (`Education × Female` = `r or_logit_int`, `r p_logit_label`) is **`r sig_logit`**. An OR `r ifelse(or_logit_int < 1, paste0("< 1 means each year of education multiplies women's odds of union membership by less than men's (specifically, by a factor of ", or_logit_int, " relative to men's OR of ", or_logit_educ, ") — so the lines converge as education rises"), paste0("> 1 means each year of education multiplies women's odds by more than men's (specifically, by a factor of ", or_logit_int, " relative to men's OR of ", or_logit_educ, ") — so the lines diverge as education rises"))`. This is why the probability curves `r ifelse(or_logit_int < 1, "converge", "diverge")` at higher education levels in the plot.
...
- **Continuous × binary interaction:**
  ...
  | Effect of X for key group | $\hat\beta_1 + \hat\beta_3$ |
...
- **In-class presentations** — <span class="highlight">April 28/May 5</span>
