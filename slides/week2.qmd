...
## Converting variable types {.smaller}
...
-   Can convert character → numeric, but only if the original character vector consists of numbers, using the `as.numeric` function
...
## Tidyverse in Practice: `attain` Data {.smaller}

Let's practice some of these commands together using the `attain` data we'll be using for our homework. 

Follow these steps to get set up so we can follow along: 

1. Download `lab1.qmd` and `attain.csv` from bCourse under "labs" >  "Lab #1"
2. Create `soc16` folder on your desktop (or another place if you prefer)
3. Create `data` and `lab` folders inside of `soc16`
4. Data always goes inside data folder and assignment always goes instead `lab`
5. Open Positron, click the `Open Folder` button and navigate to `soc16`
6. Allow access, check "Trust the authors..", and click "Yes, ..."
7. Use `Explorer` button on left to find `lab1.qmd`

**Now you're ready to code along with me!**
...
Lecture is based on the first two RFundamentals classes, which are run at the [D-Lab](https://dlab.berkeley.edu/home)
