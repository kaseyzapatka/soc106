...
- How to study efficiently this week without trying to memorize everything
...
- A margins plot helps you describe the pattern visually, even before you write out the coefficients

::: {.callout-note icon=false}
## What to listen for in your own answer
You should be able to say whether the relationship gets stronger, weaker, steeper, flatter, or changes direction across groups.
:::


## Moderation vs. Mediation {.smaller}

| | Moderation | Mediation |
|---|---|---|
| Big question | **For whom** or **under what conditions** does `X` affect `Y`? | **How** or **why** does `X` affect `Y`? |
| Third variable | Changes the slope | Carries the effect |
| Typical model | Interaction term | Pathway story |
| Visual idea | Non-parallel lines | `X -> M -> Y` |
| Example | Education predicts income differently for men and women | Education raises income partly through job prestige |

::: {.callout-tip icon=false}
## Quick test
Draw the arrows. If the third variable is **between** `X` and `Y`, think mediation. If it **changes the slope** of `X -> Y`, think moderation.
:::
...
- Revisit the slides and assignments on these topics:
  ...
  - Week 13: interactions and margins plots
  - Week 14: mediation versus moderation
