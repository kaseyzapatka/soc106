...
Population → Sample → Statistic → Sampling distribution → **Confidence interval → Inference**
:::
...
- An estimator is **efficient** if its sampling variability is lowest among alternatives
...
2. A **standard error**: how uncertain is that estimate?
3. A **critical value**: how many SEs do we need to cover 95% of outcomes?
...
It is tempting to say "there's a 95% chance the true value is in this interval" — but this is wrong. The true parameter $\pi$ is a fixed (unknown) number; it doesn't have a probability of being anywhere. **Rather**, it is the *interval itself* that varies from sample to sample. The correct interpretation: if we repeated this study 100 times and built a CI each time, 95 of those intervals would contain the true $\pi$. Any single interval either contains it or doesn't.
:::
...
## Pause & Predict {.smaller}

The GSS environmental survey has $\hat{p} = 0.449$, $n = 1154$, and a 95% margin of error of **±0.029**.

**If the sample size doubled to $n = 2308$ (same $\hat{p}$), what would happen to the margin of error?**

::: {.fragment .fade-up}
**A)** It would roughly double (get bigger)

**B)** It would stay about the same

**C)** It would shrink to about half (±0.014)

**D)** It would shrink by about 30% (±0.020)
:::

::: {.fragment .fade-up}
::: {.callout-note icon=false}
## Answer: D — it shrinks by ~30%

$$ME = z^* \times \sqrt{\frac{\hat{p}(1-\hat{p})}{n}} \qquad \text{doubling } n \text{ divides } ME \text{ by } \sqrt{2} \approx 1.41$$

Precision is expensive: you need **4× the data** to halve the margin of error. A sample of 4616 would give ±0.014.
:::
:::
...
For a **95% CI** with $n = 100$ observations:
...
Compare to $z^* = 1.960$ — very close when $n$ is large!
...
df_val  <- 6         # heights example: n = 7, df = 6
t_crit  <- qt(0.975, df = df_val)   # ≈ 2.447
t_obs   <- 3.3       # a hypothetical statistic in the rejection region
...
ggplot(df_tcurve, aes(x)) +
  ...
  # observed t-statistic in rejection region
  geom_point(data = tibble(x = t_obs, y = 0), aes(x = x, y = y),
             color = "#F28E2B", size = 5) +
  annotate("text", x = t_obs, y = dt(t_obs, df_val) + 0.030,
           label = paste0("t = ", t_obs, "\n(rejection region)"),
           color = "#F28E2B", size = 2.8, hjust = 0.5) +
  # t* label
  ...
  labs(x = "t", y = "Density",
       title = paste0("t-distribution (df = ", df_val, "): t* = ",
                      round(t_crit, 3), " | Orange dot falls in rejection region")) +
  theme_minimal(base_size = 10) +
...
## Example: Heights {.smaller .scrollable}
...
qt(0.975, df = 6)   # two-tailed: 0.025 in each tail
```
...
- Estimating a **population proportion** (large samples)
- Population standard deviation $\sigma$ is actually known (rare!)
- Very large samples ($n > 100$) where $t \approx z$
...
| **Critical value** | $z^*$ from $N(0,1)$ | $t^*$ from $t(df = n-1)$ |
| **SE** | $\sqrt{\hat{p}(1-\hat{p})/n}$ | $s/\sqrt{n}$ |
| **R function** | `qnorm()` | `qt()` |
...
# For a proportion CI — use qnorm (Normal distribution)
qnorm((1 - 0.95) / 2, lower.tail = FALSE)   # z* for 95% CI

# For a mean CI — use qt (t-distribution), specify degrees of freedom
qt((1 - 0.95) / 2, df = 99, lower.tail = FALSE)   # t* for 95% CI, n = 100
```
