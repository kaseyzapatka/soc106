...
- **Years of Education (`r b_educ_logit`):** for each additional year of education, the **log-odds** of union membership change by `r b_educ_logit` — the `r ifelse(b_educ_logit < 0, "negative", "positive")` sign means more education is associated with *`r ifelse(b_educ_logit < 0, "lower", "higher")`* odds of union membership
...
**Odds Ratios column:** OR = `r or_educ`. Values < 1 mean lower odds; values > 1 mean higher odds. An OR of `r or_educ` per year of education means: each additional year is associated with a **`r abs(pct_chg_educ)`% `r ifelse(pct_chg_educ < 0, "decrease", "increase")` in the odds** of union membership.
