...
**Weekly Assignment #10** *(last weekly assignment!)*
...
::: {.fragment .fade-up}
| $P(Y=1)$ | Odds | Log-odds (logit) | Plain reading |
|---|---|---|---|
...
The table above shows raw **odds** (the ratio $P/(1-P)$ for one group or one value of X). An **odds ratio** compares the odds from two groups — it is the *ratio of two odds*: $OR = \text{Odds}_{\text{group A}} / \text{Odds}_{\text{group B}}$. Log-odds is simply $\log(\text{Odds})$, which is what the logistic regression model predicts directly.
:::
:::
...
These are OLS-specific assumptions that do not apply to logistic regression — another advantage of using the right model for binary outcomes.
:::
:::
...
- Negative $\beta$ (`r b_educ_logit < 0`): more education → **lower** probability of union membership
...
Adding `sex_f` as a control variable:

```{r}
#| echo: false
#| eval: true

model_logit_multi <- glm(union_member ~ educ + sex_f,
                         family = binomial,
                         data   = attain_logit)

modelsummary(
  list("Bivariate" = model_logit_biv, "Multiple" = model_logit_multi),
  exponentiate = TRUE,
  coef_rename  = c("(Intercept)"  = "Intercept",
                   "educ"         = "Years of Education",
                   "sex_ffemale"  = "Female"),
  stars   = TRUE,
  title   = "Odds Ratios: Union Membership",
  gof_map = c("nobs", "AIC")
)
```
...
::: {.callout-note icon=false}
## Interpretation:
**Years of Education:** After controlling for sex, each additional year of education is associated with a `r abs(round((exp(coef(model_logit_multi)[["educ"]]) - 1) * 100, 1))`% `r ifelse(exp(coef(model_logit_multi)[["educ"]]) < 1, "decrease", "increase")` in the odds of union membership (OR = `r round(exp(coef(model_logit_multi)[["educ"]]), 3)`), holding sex constant.

**Female:** Women have `r abs(pct_chg_sex)`% `r ifelse(or_sex_female < 1, "lower", "higher")` lower odds of union membership than men (1- `r or_sex_female`), after controlling for education — consistent with occupational segregation patterns in 1991 GSS data.
:::
...
- Going from `educ = 4` to `educ = 5` → small change in probability (if you're already near the floor/ceiling)
- Going from `educ = 10` to `educ = 11` → larger change in probability (steeper part of the sigmoid)

The odds ratio gives **one summary number** that averages over this variation. But showing **predicted probabilities at specific values of X** is often more interpretable and more honest.
:::

::: {.fragment .fade-up}
::: {.callout-note icon=false}
## For multiple regression models: "hold others constant"
When you have more than one predictor, the predicted probability of Y = 1 depends on the values of all predictors. The standard approach: hold all other continuous variables at their sample mean, and all other categorical variables at their most common category (or specify the group of interest), then vary the key predictor.
:::
:::
...
**Weekly Assignment #10** — <span class="highlight">Due Thursday, April 23</span> *(last weekly assignment!)*
