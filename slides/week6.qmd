...
## Quick Review: Random Variables and Distributions {.smaller}
...
|-|---|---|---|
| **`d`** | **d**ensity / probability at a point | "Probability of **exactly** this value?" | `dbinom(6, size = 10, prob = 0.6)` |
| **`p`** | cumulative **p**robability | "Probability of **at most** this value?" | `pbinom(6, size = 10, prob = 0.6)` |
...
ggplot(tibble(x = c(40, 80)), aes(x)) +
  ...
  annotate("text", x = 60, y = 0.04, label = "84.5%", size = 5, fontface = "bold") +
...
ggplot(tibble(x = c(40, 80)), aes(x)) +
  ...
  annotate("text", x = 73, y = 0.015, label = "2.0%", size = 5, fontface = "bold", color = "#E15759") +
...
ggplot(tibble(x = c(40, 80)), aes(x)) +
  ...
  annotate("text", x = 55, y = 0.04, label = "95%", size = 5, fontface = "bold") +
...
```{r}
#| echo: true
# P(68 ≤ height ≤ 70) for each group
p_men   <- pnorm(70, mean = 70, sd = 3) - pnorm(68, mean = 70, sd = 3)
p_women <- pnorm(70, mean = 64, sd = 3) - pnorm(68, mean = 64, sd = 3)

p_men - p_women   # difference in proportions
p_men / p_women   # relative risk ratio
```

Same R functions, just applied to **two different distributions** — then we compare the results.
:::
