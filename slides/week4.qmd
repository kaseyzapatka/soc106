...
## What makes a good research question? {.smaller}

A strong social science research question has several key components:
...
## Skewness and central tendency {.smaller}
...
> Also, remember there is a big difference between "percentage point increase" and "percent increase"
...
## Correlation examples {.smaller}

What do different correlation strengths look like? Note: $r = 0$ does **not** imply no relationship — only no **linear** relationship. Always use a scatterplot to check!
...
gen_corr <- function(r, n = 200) {
  ...
  tibble(x = x, y = y, correlation = paste0("r = ", r))
}
...
corr_data <- bind_rows(
  ...
  mutate(correlation = factor(correlation,
    levels = c("r = 0", "r = 0.3", "r = 0.5", "r = 0.8")))
...
1. Download `lab4.qmd` from bCourse under "assignments" > "Lab #4"
2. Place `lab4.qmd` in your `labs` folder. Your folder structure should now look like this:
